                ...
                if (item.label === 'Services') {
                  return (
                    <li key={item.label} className="relative group h-full flex items-center" aria-haspopup="true">
                      ...
                      <div className="absolute top-[calc(100%+0.5rem)] -left-4 w-56 lg:w-60 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform group-hover:translate-y-0 translate-y-2 z-50">
                         {/* Container */}
                            ...
                            {SERVICE_CATEGORIES.map((category) => (
                              <div key={category.id} className="relative group/item px-2">
                                 ...
                                 <Link 
                                     ...
                                     className="flex items-center justify-between px-3 py-2 rounded-lg text-textLight hover:text-dark hover:bg-gray-50 transition-colors group-hover/item:bg-gray-50 group-hover/item:text-dark"
                                 >
                                     <div className="flex items-center gap-2">
                                         <div className="w-7 h-7 rounded-full bg-gray-50 flex items-center justify-center text-primary group-hover/item:bg-white group-hover/item:shadow-sm transition-all shrink-0">
                                            {/* Standardized icon size */}
                                             <category.icon size={14} />
                                         </div>
                                         <span className="font-semibold text-xs leading-tight">{category.title}</span>
                                     </div>
                                 ...
                                 <div className="absolute top-0 left-[95%] w-52 pl-2 opacity-0 invisible group-hover/item:opacity-100 group-hover/item:visible transition-all duration-200 transform group-hover/item:translate-x-0 -translate-x-2 z-50">
                                     <div className="bg-gray-50 rounded-xl shadow-2xl border border-gray-200 p-2 border-l-4 border-l-secondary">
                                         ...
                                         <div className="space-y-0.5">
                                             {category.offerings.map(offering => (
                                                 <Link 
                                                     ...
                                                     className="block px-3 py-1.5 rounded-lg text-xs text-textLight font-medium hover:text-primaryDark hover:bg-white hover:shadow-sm transition-all duration-200"
                                                 >
                                                     {offering.title}
                                                 </Link>
                                             ))}
                                         ...
                                         <div className="mt-2 pt-2 border-t border-gray-200">
                                             <Link 
                                                 to={`/services/${category.id}`} 
                                                 className="flex items-center justify-between px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-primaryDark hover:text-dark transition-colors"
                                             >
                                                 <span>View All</span>
                                                 <ArrowRight size={12} />
                                             </Link>
